...
// csvDataContinentWise
                ...
                Object.keys(csvDataObject).forEach(function(key) {
                    ...
                    Object.keys(csvDataObject[key]).forEach(function(k){
                        ...
                        var fs = require('fs');
                        fs.writeFile("csvData"+ file_postfix +".json", JSON.stringify(arrTemp), function(err) {
                            if(err) {
                ...
                var fs = require('fs');
                fs.writeFile("csvDataAfrican.json", JSON.stringify(arrVar), function(err) {
                    if(err) {
                        return console.log(err);
                    }
                ...
                Object.keys(csvDataContinentWise).forEach(function(key) {
                    console.log(key + " -->" + csvDataContinentWise[key]);
                         arrVarCon.push(
                             {'x' : key, 'y' : csvDataContinentWise[key]}
                         );
                });
                var fs = require('fs');
                fs.writeFile("csvDataContinent.json", JSON.stringify(arrVarCon), function(err) {
                    if(err) {
                        return console.log(err);
                    }
